...
Template.page1.events({
  ...
  "change #profile_location"(event) {
    const target = event.target;
    const value = target.value;
    Session.set("profile_location", value);
  },
  "click #next"(event) {
...
Template.comparissionTemplate.onRendered(function () {
  $(document).on("input", "#cus_range", function () {
    ...
    if ($(this).val() >= 48 && $(this).val() <= 52) {
      ...
      if ($(this).val() <= 30) {
        $("#medidor").text("Prefiro a alternativa de baixo");
      }
      if ($(this).val() <= 10) {
        $("#medidor").text("Prefiro MUITO a alternativa de baixo");
      }
    } else if ($(this).val() > 52) {
      $("#medidor").text("Prefiro ligeiramente a de cima");
      $("#medidor_img").addClass("fas fa-arrow-circle-up text-info fa-lg");
      if ($(this).val() >= 70) {
        $("#medidor").text("Prefiro a alternativa de cima");
      }
      if ($(this).val() >= 90) {
        $("#medidor").text("Prefiro MUITO a alternativa de cima");
      }
    }
  });
});
